...
이 프로토타입은 리깅, 관절 좌표, 잔상, 임팩트 스냅, 컷신 타이밍을 검증하기 위한 내부 참고 구현이다. Animotion의 제품 방향은 오리지널 또는 정식 권리를 확보한 캐릭터를 직접 리깅하는 제작툴이며, 이 폴더의 특정 컷/스타일을 제품 데모나 무단 IP 워크플로우로 확장하지 않는다.
